...
namespace COM3D2.VoiceShortcutManager.Plugin
{
    ...
    public class VoiceShortcutManager : PluginBase
    {
        ...
        Dictionary<string, List<YotogiVoiceInfo>>
            yotogiVoiceKeywoards = new Dictionary<string, List<YotogiVoiceInfo>>();
        ...
        public class MenuInfo
        {
            ...
            public MenuInfo(string name, string label, string[] voice, Keys key, bool shift, bool ctrl, bool alt,
                string sceneRegex, string sceneId)
            {
                this.name = name;
                this.label = label;
                this.voice = voice;
        ...
        void Start()
        {
            ...
            try
            {
                ...
                {
                    if (cfg.vymEnabled)
                        Debug.LogError("[" + GetType().Name + "] VibeYourMaid vymGetVersion() not found");
                    vymType = null; //無効化
                }
                else
                {
        ...
        void OnLevelWasLoaded(int level)
        {
            ...
            if (cfg.keyboardEnabled && !this.keyboardButton)
            {
                ...
                this.keyboardButton =
                    GearMenu.Buttons.Add("Keyboard", cfg.keyboardLabelOFF, icon, (go) => toggleKeyboard());
            }
        ...
        void Update()
        {
            ...
            if (this.keyboardVisible && this.guiStopPropagation)
            {
                Vector2 pos = new Vector2(Input.mousePosition.x,
                    (float)UnityEngine.Screen.height - Input.mousePosition.y);
        ...
        private void loadConfig()
        {
            ...
            if (System.IO.File.Exists(configFile))
            {
                ...
                try
                {
                    ...
                    using (System.IO.StreamReader sr =
                           new System.IO.StreamReader(configFile, new System.Text.UTF8Encoding(false)))
                    {
                    ...
                    if (cfg.version != VERSION)
                    {
                        ...
                        using (System.IO.StreamWriter sw =
                               new System.IO.StreamWriter(configFile, false, new System.Text.UTF8Encoding(false)))
                        {
            ...
            {
                //サンプル設定
                cfg.menuList.Add(new MenuInfo("PropMyItem", "PropMyItem", null, Keys.I, true, false, false, null,
                    null));
                cfg.menuList.Add(new MenuInfo("ModsSlider", "ModsSlider", null, Keys.F5, false, false, false,
                    "^SceneEdit$", null));
                cfg.menuList.Add(new MenuInfo("YotogiSlider", "YotogiSlider", null, Keys.F5, false, false, false,
                    "^SceneYotogi$", null));
                cfg.menuList.Add(new MenuInfo("Pause", "Pause", new string[] { "ポーズ", "停止" }, Keys.Enter, false, false,
                    false, "^(SceneDance_|SceneVR)", null));
                //保存
                cfg.version = VERSION;
                System.Xml.Serialization.XmlSerializer serializer =
                    new System.Xml.Serialization.XmlSerializer(typeof(Config));
        ...
        public void toggleKeyboard()
        {
            ...
            Vector2 pos = new Vector2(Input.mousePosition.x,
                (float)UnityEngine.Screen.height - Input.mousePosition.y + cfg.keybordOffsetY);
            gui.rect.x = pos.x - 10;
            gui.rect.y = pos.y - 10;
        }
        ...
        {
            return (menu.sceneRegex == null || Regex.IsMatch(sceneName, menu.sceneRegex)) && (menu.sceneId == null ||
                Regex.IsMatch(sceneLevel.ToString(), "^" + menu.sceneId + "$"));
        }
        ...
        private void loadVoiceConfig()
        {
            ...
            {
                System.Xml.Serialization.XmlSerializer serializer =
                    new System.Xml.Serialization.XmlSerializer(typeof(VoiceConfig));
                using (System.IO.StreamReader sr =
                       new System.IO.StreamReader(voiceConfigFile, new System.Text.UTF8Encoding(false)))
                {
        ...
        private void saveVoiceConfig()
        {
            System.Xml.Serialization.XmlSerializer serializer =
                new System.Xml.Serialization.XmlSerializer(typeof(VoiceConfig));
            using (System.IO.StreamWriter sw =
                   new System.IO.StreamWriter(voiceConfigFile, false, new System.Text.UTF8Encoding(false)))
            {
        ...
        private void loadVymConfig()
        {
            ...
            {
                System.Xml.Serialization.XmlSerializer serializer =
                    new System.Xml.Serialization.XmlSerializer(typeof(VymConfig));
                using (System.IO.StreamReader sr =
                       new System.IO.StreamReader(vymConfigFile, new System.Text.UTF8Encoding(false)))
                {
        ...
        private void saveVymConfig()
        {
            System.Xml.Serialization.XmlSerializer serializer =
                new System.Xml.Serialization.XmlSerializer(typeof(VymConfig));
            using (System.IO.StreamWriter sw =
                   new System.IO.StreamWriter(vymConfigFile, false, new System.Text.UTF8Encoding(false)))
            {
        ...
        private void enableMic()
        {
            ...
            if (keywords.Count() == 0)
            {
                ...
                if (this.icons.ContainsKey("MicDisabled"))
                    GearMenu.Buttons.SetTexture(micButton, this.icons["MicDisabled"]);
            }
            else
            {
                //ラベルとアイコン変更
        ...
        private void setVoiceKeywords()
        {
            ...
            {
                keywords.Add("テスト", () => { Debug.Log("Voice Test : テスト"); });
            }
        }
        ...
        private void setYotogiKeywords()
        {
            ...
            this.yotogiCommandUnit =
                GameObject.Find(
                    "/UI Root/YotogiPlayPanel/CommandViewer/SkillViewer/MaskGroup/SkillGroup/CommandParent/CommandUnit");
        ...
        public void playVoice(int skipCount)
        {
            ...
            {
                FieldInfo backlogUnitInfo = typeof(MessageWindowMgr).GetField("m_listBacklogUnit",
                    BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.GetField);
                List<BacklogCtrl.BacklogUnit> backlogList =
                    (List<BacklogCtrl.BacklogUnit>)backlogUnitInfo.GetValue(GameMain.Instance.MsgWnd);
                //新しい順
                for (int i = backlogList.Count - 1; i >= 0; i--)
                {
                    ...
                    else
                    {
                        ...
                        //メイドが見つからなかったら通常再生
                        GameMain.Instance.SoundMgr.PlayDummyVoice(unit.m_voiceId, 0f, false, false, 50,
                            AudioSourceMgr.Type.Voice);
                        return;
                    }
                }
        ...
        private void setVymKeywoards()
        {
            ...
            if (cfg.vymCamMoveEnabled)
            {
                ...
                foreach (VymVoiceInfo info in vymCfg.vymCamTarget)
                {
                    ...
                    setVymKeywordInvoke(info.voice, "vymCamTarget", info.value + " " + info.value2,
                        new object[] { value, Math.Abs(value2), value2 < 0 });
                }
                ...
                foreach (VymVoiceInfo info in vymCfg.vymCamManHead)
                {
                    ...
                    setVymKeywordInvoke(info.voice, "vymCamManHead", info.value + " " + info.value2,
            ...
            foreach (VymVoiceInfo info in vymCfg.vymFollow)
            {
                ...
                setVymKeywordInvoke(info.voice, "vymLookPoint", "Follow " + info.value,
                    new object[] { -1, follow, -1 });
            }
            ...
            foreach (VymVoiceInfo info in vymCfg.vymVibeAuto)
            {
                int value = int.Parse(info.value);
                bool offset = info.value.StartsWith("+") || info.value.StartsWith("-");
                setVymKeywordInvoke(info.voice, "vymVibeAuto", info.value,
                    new object[] { value, offset, cfg.vymVibeLinkType });
            }
        ...
        private string vymGetVersion()
        {
            return CM3D2.VibeYourMaid.Plugin.API.vymGetVersion();
        }

        private Maid vymGetFrontMaid()
        {
            return CM3D2.VibeYourMaid.Plugin.API.vymGetFrontMaid();
        }

        private List<Maid> vymGetMaidList(int linkType)
        {
            return CM3D2.VibeYourMaid.Plugin.API.vymGetMaidList(linkType);
        }

        private void vymFace(string face, string blend, float fadeFace)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymFace(face, blend, fadeFace, cfg.vymFaceLinkType);
        }

        private void vymFaceBlend(string[] tags)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymFaceBlend(tags, cfg.vymFaceLinkType);
        }
        ...
        private void vymVoiceSet(string voiceFile)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymVoiceSet(voiceFile);
        }
        ...
        private void vymGetConfig(VymConfigInfo info)
        {
            ...
            {
                //VYM設定取得
                CM3D2.VibeYourMaid.Plugin.VibeYourMaid.VibeYourMaidCfgWriting cfgw =
                    CM3D2.VibeYourMaid.Plugin.API.vymGetConfig();
                //データ型に合わせて設定
        ...
        {
            CM3D2.VibeYourMaid.Plugin.API.vymSelectFrontMaid();
        }

        private void vymSelectMaidName(string name)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymSelectMaidName(name);
        }

        private void vymPrevMaid(bool camChange)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymPrevMaid(camChange);
        }

        private void vymNextMaid(bool camChange)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymNextMaid(camChange);
        }

        private void vymLeftMaid(bool camChange)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymLeftMaid(camChange);
        }

        private void vymRightMaid(bool camChange)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymRightMaid(camChange);
        }

        private void vymMaidLink(string type)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymMaidLink(type);
        }

        private void vymManVisible(int manVisible)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymManVisible(manVisible);
        }

        private void vymCamMove(Vector3 move, bool useMoveValue)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymCamMove(move, useMoveValue);
        }

        private void vymCamDistance(float distance, int moveType)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymCamDistance(distance, moveType);
        }

        private void vymCamAround(int aroundType, float angleOffset)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymCamAround(aroundType, angleOffset);
        }

        private void vymCamTarget(float distance, int target, bool backword)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymCamTarget(distance, target, backword);
        }

        private void vymCamManHead(int manID, int target)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymCamManHead(manID, target);
        }

        private void vymLookPoint(int fpsMod, int follow, int lookPoint)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymLookPoint(fpsMod, follow, lookPoint);
        }
        ...
        {
            CM3D2.VibeYourMaid.Plugin.API.vymExcite(excite, offset);
        }

        private void vymSyasei(int man, bool lockMode)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymSyasei(man, lockMode);
        }

        private void vymFukitori(int mode)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymFukitori(mode);
        }

        private void vymStartNyo(int linkType)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymStartNyo(linkType);
        }

        private void vymStartSio(int linkType)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymStartSio(linkType);
        }

        private void vymVibe(int level)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymVibe(level, cfg.vymVibeLinkType);
        }

        private void vymVibeAuto(int auto, bool offset)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymVibeAuto(auto, offset, cfg.vymVibeLinkType);
        }

        private void vymKupa(float kupa, bool offset)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymKupa(kupa, offset);
        }

        private void vymAnal(float kupa, bool offset)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymAnal(kupa, offset);
        }

        private void vymBokki(float bokki, bool offset)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymBokki(bokki, offset);
        }

        private void vymUnzip(string motion)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymUnzip(motion);
        }

        private void vymUnzipDerive(string[] derives)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymUnzipDerive(derives);
        }

        private void vymUnzipRandom(string emsFile)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymUnzipRandom(emsFile);
        }

        private void vymUnzipChange(int changeTYpe)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymUnzipChange(changeTYpe);
        }

        private void vymRemoveMotion(bool syasei)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymRemoveMotion(syasei);
        }

        private void vymInsertMotion()
        {
            CM3D2.VibeYourMaid.Plugin.API.vymInsertMotion();
        }

        private void vymSumataMotion()
        {
            CM3D2.VibeYourMaid.Plugin.API.vymSumataMotion();
        }

        private void vymAnalMode(bool analMode)
        {
            CM3D2.VibeYourMaid.Plugin.API.vymAnalMode(analMode);
        }

        private void vymKissVoiceSet(string voiceFile)
        {
        ...
        private UndressingManager getUndressingManager()
        {
                ...
                YotogiManager.instans.play_mgr.playingSkill != null)
            {
                GameObject obj =
                    GameObject.Find(
                        "UI Root/YotogiPlayPanel/UndressingViewer/UndressingViewer/MaskGroup/UndressParent");
        ...
        public void dressAcc()
        {
            ...
            if (undressManager != null)
            {
                undressManager.SetMaskMode(UndressingManager.UnitType.ヘッドドレス,
                    UndressingManager.MaskStatus.Off); //Offが表示
        ...
        private bool PororiChange(Maid maid, string name, TBody.SlotID slotID, MPN mpn)
        {
            ...
            else if (prop.nTempFileNameRID != 0 && !prop.strTempFileName.Contains("_porori"))
            {
                //仮衣装でぽろり無し
